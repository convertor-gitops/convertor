...
export class ConvUrl implements Cloneable<ConvUrl>, Equatable<ConvUrl>, Serializable {
  ...
  public clone(): ConvUrl {
    return new ConvUrl(this.type.clone(), this.server, this.query?.clone() ?? null);
  }
  ...
  public serialize(): any {
    return {
      ...
      server: this.server,
      query: this.query?.serialize() ?? null,
    };
  }

  public static deserialize(value: unknown, path = '$'): ConvUrl {
    const conv_url = asObject(value, path);
    return new ConvUrl(
      UrlType.deserialize(required(conv_url, 'type', path), childPath(path, 'type')),
      asString(required(conv_url, 'server', path), childPath(path, 'server')),
      optionalNullable(conv_url['query'], ConvQuery.deserialize, childPath(path, 'query')) ?? null,
    );
  }
}
...
export class UrlType implements Cloneable<UrlType>, Equatable<UrlType>, Serializable {
  ...
  public static readonly Original = new UrlType(
    UrlTypeVariant.Original,
    '/original',
    '订阅商原始订阅配置',
    'original',
  );
  public static readonly Raw = new UrlType(
    UrlTypeVariant.Raw,
    '/raw',
    '转换前订阅配置',
    'raw_profile',
  );
  public static readonly Profile = new UrlType(
    UrlTypeVariant.Profile,
    '/profile',
    '转换后订阅配置',
    'profile',
  );
  public static readonly ProxyProvider = new UrlType(
    UrlTypeVariant.ProxyProvider,
    '/proxy-provider',
    '代理提供者',
    'proxy_provider',
  );
  public static readonly RuleProvider = new UrlType(
    UrlTypeVariant.RuleProvider,
    '/rule-provider',
    '规则提供者',
    'rule_provider',
  );
  ...
  public static deserialize(value: unknown, path = '$'): UrlType {
    const type = UrlType.variants().find((v) => v.variant === value);
    if (type === undefined) {
      throw new DeserializeError(path, `unknown UrlType ${JSON.stringify(value)}`);
    }
    return type;
  }
}
