...
import { CAPTURED_EXCHANGE, CapturedExchangeHolder } from '../common/http/captured-exchange';
...
export class DashboardService {
  ...
  error: BehaviorSubject<DashboardErrorResponse | null> =
    new BehaviorSubject<DashboardErrorResponse | null>(null);
  ...
  public healthCheck(): Observable<ResponseBody | null> {
    return this.http
      .get<ResponseBody>(DashboardService.HEALTH_ENDPOINT)
      .pipe(map((response) => ResponseBody.deserialize(response)));
  }

  public redisCheck(): Observable<ResponseBody<string> | null> {
    return this.http
      .get<ResponseBody<string>>(DashboardService.REDIS_ENDPOINT)
      .pipe(map((response) => ResponseBody.deserialize<string>(response)));
  }
  ...
  public getSubscription(query: ConvQuery): Observable<ResponseBody<UrlResult>> {
    ...
    return this.http.get(path, { context }).pipe(
      map((response) => {
        const body = ResponseBody.deserialize(response, UrlResult.deserialize);
        if (body === null) {
