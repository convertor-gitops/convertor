...
impl ConvQuery {
    pub fn decrypt(mut self, secret: impl AsRef<str>) -> Result<Self, QueryError> {
        let secret = secret.as_ref();
        self.secret = self.secret.take().map(|s| decrypt(secret.as_bytes(), s.as_str())).transpose()?;
        self.sub_url = decrypt(secret.as_bytes(), self.sub_url.as_str())?;
        Ok(self)
    }
