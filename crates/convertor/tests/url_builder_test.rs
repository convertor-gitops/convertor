...
fn test_build_surge_url() -> color_eyre::Result<()> {
    ...
    insta::assert_snapshot!(raw_url.to_string(), @"http://127.0.0.1:8080/raw?server=http%3A%2F%2F127.0.0.1%3A8080%2F&client=surge&interval=86400&strict=true&sub_url=iImwC4XKfx_wFPtS_z90X5RlgwMLZx_rEww0IyRDliwS0iO2A75TY4z2P2NQvS26yRldrv67GWkRyABL4uw7VAVN6ebUkKBGjn4Xyp3AYWBcH8j5bKF_54nHIS5r5pHBQz0");

    let profile_url = url_builder.build_profile_url()?;
    insta::assert_snapshot!(profile_url.to_string(), @"http://127.0.0.1:8080/profile?server=http%3A%2F%2F127.0.0.1%3A8080%2F&client=surge&interval=86400&strict=true&sub_url=88wW1Vq-k8ffi8eWsfLouIQmLD4YasouA_9tQhI36EeDf-w3wukERaFAZZL7Am_g0SqmcJJ5CMgzLLhkbceOSmq54bP-OiN92hHI-uPmXj3156iy20qtxmfO82pxpdm_lFM");
...
fn test_build_clash_url() -> color_eyre::Result<()> {
    ...
    let raw_url = url_builder.build_raw_url()?;
    insta::assert_snapshot!(raw_url.to_string(), @"http://127.0.0.1:8080/raw?server=http%3A%2F%2F127.0.0.1%3A8080%2F&client=clash&interval=86400&strict=true&sub_url=SfxJSAferU_ckMID-y4NAqzOwSEai63Q29zkYoBTPL5UWDT7IKdsYBb4ZpxG1kPOTTb2qYeQVUgvBIvLaqawonXysSI1B_7u4RPKmjw-iPrSyPhPoDVxiwi0K04KF82QNVU");

    let profile_url = url_builder.build_profile_url()?;
    insta::assert_snapshot!(profile_url.to_string(), @"http://127.0.0.1:8080/profile?server=http%3A%2F%2F127.0.0.1%3A8080%2F&client=clash&interval=86400&strict=true&sub_url=ydgGuWMN4VNtPe__koDVXi2nZPSRnfIGAsDpI1HBcpmaHt1QeeFKlR0ISwVcdsMZf8FQfNHxLM9_yh7tH7ja_MaYjgalH0gg69zKlObGm_Qwk6EvANV2UhDiuEp8i4h89fA");
